  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground text-sm mt-1">Colecciona badges e mostra o teu progresso</p>
      </motion.div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-display font-bold text-foreground">Colecção</span>
              </div>
...
function CategorySection({
  catKey, cat, badges, progress, onSelect,
}: {
  catKey: BadgeCategory;
  cat: { label: string; icon: string; color: string };
  badges: CollectibleBadge[];
  progress: { total: number; unlocked: number; pct: number };
  onSelect: (b: CollectibleBadge) => void;
}) {
  return (
    <div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
        {badges.map((badge, i) => (
          <BadgeCard key={badge.id} badge={badge} index={i} onSelect={onSelect} />
        ))}
      </div>
    </div>
  );
...
function BadgeCard({ badge, index, onSelect }: { badge: CollectibleBadge; index: number; onSelect: (b: CollectibleBadge) => void }) {
  ...
  return (
    ...
    >
      <Card
        className={`cursor-pointer transition-all duration-300 hover:-translate-y-1 active:scale-95 border-border/50 ${
          isUnlocked ? `hover:shadow-lg ${tierConfig.glow}` : 'opacity-60'
        }`}
      ...
      >
        <CardContent className="p-3 text-center">
          ...
          <Badge className={`text-[8px] mt-1 border-0 px-1.5 py-0 ${tierConfig.bg} ${tierConfig.color}`}>
            {tierConfig.label}
          </Badge>
        </CardContent>
      </Card>
      ...
      <CardContent className="p-0">
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="bg-muted/50 rounded-xl p-3">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold mb-1">Como desbloquear</p>
            <p className="text-xs text-foreground">{badge.requirement}</p>
          </div>
          {isUnlocked && badge.unlockedAt && (
            <div className="flex items-center gap-2 text-xs">
              <Sparkles className="h-4 w-4 text-primary" />
              <span className="text-chart-3 font-semibold">Desbloqueado em {badge.unlockedAt}</span>
            </div>
          )}
          <button
            onClick={onClose}
            className="w-full py-2.5 rounded-xl bg-muted hover:bg-muted/80 text-sm font-semibold text-foreground transition-colors"
          >
            Fechar
          </button>
        </div>

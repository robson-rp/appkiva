  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if ((result as any)?.success) {
        ...
        toast.error((result as any)?.error || 'Erro ao aceitar convite');
      }
    } catch {
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="max-w-sm w-full rounded-2xl text-center">
          <CardContent className="pt-8 pb-6 space-y-4">
            ...
            <h2 className="font-display text-xl font-bold text-foreground">Adesão confirmada!</h2>
            <p className="text-sm text-muted-foreground">
              Agora faz parte do programa <strong>{validation?.program_name}</strong>.
            </p>
            <Button onClick={goHome} className="rounded-xl w-full">Ir para o painel</Button>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="max-w-sm w-full rounded-2xl text-center">
          <CardContent className="pt-8 pb-6 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">{validation?.error || 'Este convite não é válido ou já expirou.'}</p>
            <Button variant="outline" onClick={goHome} className="rounded-xl w-full">Voltar</Button>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="max-w-sm w-full rounded-2xl">
        <CardHeader className="text-center pb-2">
          ...
          <CardTitle className="font-display text-xl">Convite de Programa</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 text-center">
          <div className="space-y-2">
            <p className="text-sm text-muted-foreground">Foi convidado para aderir ao programa:</p>
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <Badge variant="secondary">
                {validation.target_type === 'family' ? '👨‍👩‍👧 Família' : '🏫 Escola'}
              </Badge>
            </div>
          </div>

          <div className="flex gap-2">
            <Button variant="outline" onClick={goHome} className="flex-1 rounded-xl">
              Recusar
            </Button>
            <Button

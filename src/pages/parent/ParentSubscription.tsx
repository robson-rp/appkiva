...
const FEATURE_KEYS = [
  ...
  'custom_rewards', 'budget_exceptions', 'multi_child',
  'advanced_analytics', 'export_reports', 'real_money_wallet', 'priority_support',
];
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
          ...
          <CardContent className="p-6 relative z-10">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs uppercase tracking-widest opacity-70">{t('parent.subscription.title')}</p>
                  <h1 className="font-display text-2xl font-bold">
                    {gateLoading ? '...' : tierName ?? t('pricing.free')}
                  </h1>
                  {currentTier && (
                    <p className="text-sm opacity-80 mt-0.5">
                      {currentTier.priceMonthly > 0
                        ? `${fmtP(currentTier.id, currentTier.priceMonthly)}${t('pricing.per_month')}`
                        : t('pricing.free')}
                    </p>
                  )}
                </div>
        ...
        <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-display font-semibold">{t('common.more_features')}</h2>
            </div>
            <div className="grid gap-2">
              {FEATURE_KEYS.map((feature) => {
                const included = enabledFeatures.includes(feature);
                return (
                  <div key={feature} className={cn('flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors', included ? 'bg-primary/5' : 'bg-muted/30 opacity-50')}>
                    <div className={cn('w-6 h-6 rounded-lg flex items-center justify-center shrink-0', included ? 'bg-primary/15 text-primary' : 'bg-muted text-muted-foreground')}>
                      {included ? <Check className="h-3.5 w-3.5" /> : <span className="text-[10px] font-bold">—</span>}
                    </div>
                    <span className={cn('text-sm', included ? 'font-medium' : 'text-muted-foreground')}>
                      {t(`feature.${feature}`)}
                    </span>
                    {!included && (
                      <Badge variant="outline" className="ml-auto text-[10px] border-accent/30 text-accent-foreground">Premium</Badge>
                    )}
                  </div>
                );
              })}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="space-y-3">
        <Button onClick={() => setPaymentOpen(true)} className="w-full rounded-xl font-display h-12 text-base gap-2 bg-accent hover:bg-accent/90 text-accent-foreground">
        ...
        {!isFreeTier && lowerTiers.length > 0 && (
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {lowerTiers.map((tier) => (
                <Button key={tier.id} variant="outline" size="sm" className="rounded-xl text-xs gap-1.5" onClick={() => { setDowngradeTarget(tier); setDowngradeOpen(true); }}>
                  <ArrowDown className="h-3 w-3" /> {tier.name}
                </Button>
              ))}
            </div>
          </div>
      ...
      <PaymentSimulator open={paymentOpen} onOpenChange={setPaymentOpen} currentTierName={tierName} tiers={familyTiers} onConfirmUpgrade={upgrade} />
      ...
      <Dialog open={downgradeOpen} onOpenChange={setDowngradeOpen}>
        <DialogContent className="rounded-2xl">
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setDowngradeOpen(false)} className="rounded-xl">{t('common.cancel')}</Button>
            <Button variant="destructive" onClick={handleDowngrade} disabled={upgradeLoading} className="rounded-xl font-display gap-1.5">

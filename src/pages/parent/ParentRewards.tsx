...
export default function ParentRewards() {
  ...
  return (
    <FeatureGateWrapper allowed={rewardsAllowed || gateLoading} featureName={t('parent.rewards.title')}>
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6 max-w-5xl mx-auto">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {iconOptions.map(ic => (
                    <button key={ic} type="button" onClick={() => setIcon(ic)} className={`w-10 h-10 rounded-xl text-xl flex items-center justify-center border-2 transition-all ${icon === ic ? 'border-primary bg-primary/10 scale-110' : 'border-border/50 hover:border-border'}`}>
                      {ic}
                    </button>
                  ))}
                </div>
              ...
              <Button className="w-full rounded-xl font-display" disabled={!name || !price || createReward.isPending} onClick={handleCreate}>
      ...
      {isLoading ? (
          ...
          {Array.from({ length: 3 }).map((_, i) => (
            <Card key={i} className="border-border/50">
              <CardContent className="p-4 space-y-3">
                <div className="flex items-start justify-between"><Skeleton className="w-12 h-12 rounded-2xl" /><Skeleton className="h-5 w-16 rounded-full" /></div>
                <Skeleton className="h-4 w-32" /><Skeleton className="h-3 w-full" /><Skeleton className="h-4 w-20" />
              </CardContent>
            </Card>
          ))}
        </div>
        ...
        <motion.div variants={item} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {rewards.map((reward) => (
            <motion.div key={reward.id} variants={item} whileHover={{ y: -4 }} transition={{ type: 'spring', stiffness: 300 }}>
              <Card className="h-full border-border/50 overflow-hidden hover:shadow-lg transition-all duration-300">
                ...
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-start justify-between">
                    <div className="w-12 h-12 rounded-2xl bg-muted/50 flex items-center justify-center text-3xl">{reward.icon}</div>
                    <Badge variant="outline" className={`text-[10px] ${categoryColors[reward.category]}`}>{categoryLabels[reward.category]}</Badge>
                  </div>
                  <div>
                  ...
                  <div className="flex items-center justify-between pt-1 border-t border-border/30">
                    ...
                    <Button variant="ghost" size="sm" className="h-7 rounded-lg text-xs font-display text-destructive hover:text-destructive" disabled={deleteReward.isPending} onClick={() => deleteReward.mutate(reward.id)}>

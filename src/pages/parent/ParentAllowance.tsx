  ...
  const handleSend = async (childProfileId: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${child.displayName}: ${total} KVC (base: ${eff.baseAmount} + ${t('parent.allowance.task_bonus')}: ${taskBonusTotal}). ${t('common.balance')}: ${result.new_balance} KVC.`,
      });
    } catch (err: any) {
                  ...
                  <CardContent className="p-6 space-y-5">
                    ...
                    <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground bg-muted/40 rounded-xl p-3 border border-border/30">
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {eff.lastSentAt
                            ? `${t('parent.allowance.last_sent')} ${formatDistanceToNow(new Date(eff.lastSentAt), { addSuffix: true, locale: locale === 'pt' ? ptLocale : undefined })}`
                  ...
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] text-muted-foreground">{child?.displayName ?? t('nav.parent.children')}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-display font-bold text-sm">{vault.currentAmount} 🪙</p>
                        <p className="text-[10px] text-muted-foreground">/ {vault.targetAmount}</p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                      ...
                      <Slider
                        ...
                        onValueChange={([v]) => {
                          updateVaultRate.mutate(
                            ...
                            {
                              onSuccess: () => toast({ title: t('parent.vaults.rate_updated'), description: `${vault.name}: ${v}%` }),
                      ...
                      />
                      <div className="flex justify-between text-[10px] text-muted-foreground"><span>0%</span><span>10%</span></div>
                    </div>
                  </CardContent>
                </Card>
              );
          ...
          <div className="space-y-5 mt-2">
            <div className="space-y-2">
              ...
              <Select value={editConfig.frequency} onValueChange={(v) => setEditConfig(prev => ({ ...prev, frequency: v as 'weekly' | 'monthly' }))}>
                ...
                <SelectContent>
                  <SelectItem value="weekly">{t('parent.allowance.weekly')}</SelectItem>
                  <SelectItem value="monthly">{t('parent.allowance.monthly_freq')}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <Button className="w-full rounded-xl font-display gap-2" onClick={saveConfig} disabled={upsertConfig.isPending}>
              {upsertConfig.isPending ? t('profile.saving') : t('parent.allowance.save_config')}
            </Button>
          </div>
        </DialogContent>
      </Dialog>

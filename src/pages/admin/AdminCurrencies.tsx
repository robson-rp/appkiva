...
import { Globe, ArrowRightLeft, Pencil, Plus, Loader2, RefreshCw, Power } from 'lucide-react';
...
function CurrenciesTab() {
  ...
  const createMutation = useMutation({
    mutationFn: async (vals: { code: string; name: string; symbol: string; decimal_places: number }) => {
      const { error } = await supabase
        .from('supported_currencies')
        .insert({ ...vals, is_active: true });
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-currencies'] });
      ...
      setAddOpen(false);
      setAddForm({ code: '', name: '', symbol: '', decimal_places: 2 });
    },
    onError: (e: any) => toast.error(e.message),
  });
    ...
    <>
      <div className="flex justify-end mb-4">
        <Button onClick={() => setAddOpen(true)} className="gap-1.5" size="sm">
          <Plus className="h-4 w-4" /> Nova Moeda
        </Button>
      </div>
          ...
          <Card key={c.code} className={`border-border/50 transition-opacity ${!c.is_active ? 'opacity-50' : ''}`}>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Switch
                  checked={c.is_active}
                  onCheckedChange={(v) => toggleMutation.mutate({ code: c.code, is_active: v })}
                  className="scale-75"
                />
              </div>
            </CardHeader>
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input value={addForm.code} onChange={(e) => setAddForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} className="font-mono mt-1" maxLength={5} placeholder="Ex: JPY" />
            </div>
            <div>
              <Label>Nome</Label>
              <Input value={addForm.name} onChange={(e) => setAddForm(f => ({ ...f, name: e.target.value }))} className="mt-1" placeholder="Ex: Iene Japonês" />
            </div>
            <div>
              <Label>Símbolo</Label>
              <Input value={addForm.symbol} onChange={(e) => setAddForm(f => ({ ...f, symbol: e.target.value }))} className="mt-1" maxLength={5} placeholder="Ex: ¥" />
            </div>
            <div>
              <Label>Casas decimais</Label>
              <Input type="number" min={0} max={4} value={addForm.decimal_places} onChange={(e) => setAddForm(f => ({ ...f, decimal_places: parseInt(e.target.value) || 0 }))} className="mt-1" />
            </div>
          </div>
          <DialogFooter>

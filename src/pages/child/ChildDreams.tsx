...
import { Plus, Star, MessageCircle, Sparkles, Heart, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ChildDreams() {
  ...
  const dreams = mockDreamVaults.filter((d) => d.childId === child.id);
  const [expandedId, setExpandedId] = useState<string | null>(dreams[0]?.id ?? null);
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
        ...
        <Card className="relative overflow-hidden border-0 text-primary-foreground">
          ...
          <CardContent className="p-6 relative z-10">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 mt-4">
              ...
              <Progress value={Math.round((totalSaved / totalTarget) * 100)} className="h-2.5 bg-white/20" />
            </div>
          </CardContent>
        </Card>
      </motion.div>
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="font-display">Adicionar Sonho</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
                <Label>O que sonhas?</Label>
                <Input placeholder="Ex: Ir à Disneylândia" />
              </div>
              <div className="space-y-2">
                <Label>Descreve o teu sonho</Label>
                <Textarea placeholder="Porque é importante para ti?" />
              </div>
              <div className="space-y-2">
                <Label>Quanto precisas? (KivaCoins)</Label>
                <Input type="number" placeholder="500" />
              </div>
              <Button className="w-full rounded-xl font-display">✨ Criar Sonho</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
              ...
              <Card className="overflow-hidden border-border/50 hover:shadow-lg transition-all duration-300">
                ...
                <div className="relative bg-gradient-to-r from-muted/60 to-muted/30 p-5">
                  <div className="flex items-start gap-4">
                    <motion.div
                      className="w-16 h-16 rounded-2xl bg-card shadow-md flex items-center justify-center text-4xl border border-border/30"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-lg font-display font-semibold border ${priority.className}`}>
                          {priority.label}
                        </span>
                      </div>
                  ...
                  <div className="flex gap-2 mt-3">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="rounded-xl text-xs font-display h-8 gap-1"
                        ...
                        {dream.parentComments.map((comment) => (
                          ...
                          >
                            ...
                            <div className="flex-1 bg-card rounded-2xl rounded-tl-sm p-3 shadow-sm border border-border/30">
                              ...
                              <p className="text-[10px] text-muted-foreground mt-1.5">{comment.date}</p>
                            </div>
                          </motion.div>
                        ))}
                      </CardContent>
        ...
        <Card className="border-border/50 bg-gradient-to-r from-secondary/5 to-accent/5">
          <CardContent className="p-5 text-center space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Poupa um pouco todos os dias e vê a magia acontecer. ✨
            </p>
          </CardContent>
        </Card>
      </motion.div>
